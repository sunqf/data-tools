...
def extract_pair(html: BeautifulSoup):
    ...
    words = set([a.text.strip().lower() for tag in en_tags for a in tag.select('a[href]')]
                + [a.text.strip().lower() for tag in cn_tags for a in tag.select('a[href]')])
    ...
    async def create(cls, loop, keywords, max_worker=10):
        ...
        async with db_pool.acquire() as db_conn:
            async with db_conn.transaction():
                ...
                finished = set([r['keyword'].lower() for r in records])
