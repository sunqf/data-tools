...
arg_parser.add_argument('--src', type=str, help='scel dir')
arg_parser.add_argument('--output', type=str, help='output file')
arg_parser.add_argument('--filter', type=list, help='filter list')
...
def list_all(file_or_dir, filters=['城市信息大全']):
    if os.path.isdir(file_or_dir):
        for sub in os.listdir(file_or_dir):
            sub_path = os.path.join(file_or_dir, sub)
            if sub.endswith('.scel'):
                yield os.path.join(sub_path)
            elif os.path.isdir(sub_path) and sub not in filters:
                yield from list_all(sub_path)
...
def transform(source_file_or_dir):
    ...
    files = list(list_all(source_file_or_dir))
    ...
    if os.path.isdir(source_file_or_dir):
        if not source_file_or_dir.endswith("/"):
            source_file_or_dir += "/"
        files += glob.glob(source_file_or_dir + "*.scel", recursive=True)
