...
class Sogou:
    type = 'sogou_baike'
    # http://baike.sogou.com/Search.e?sp=S%E4%BC%BD%E7%8E%9B%E5%88%80
    search_address = 'http://baike.sogou.com'
    item_address = 'http://baike.sogou.com/v'
    ...
    async def search_work(self):
        async def _search(url):
            if url not in self.search_urls:
                ...
                if html:
                    html = BeautifulSoup(html, 'html.parser')
                    for tag in html.select('#content > div > ul > li > h2 > a'):
                    ...
                    for tag in html.select('#content > div > div.pagination > ul > li > a'):
        ...
        while True:
            ...
            try:
                url = 'http://baike.sogou.com/Search.e?sp=S{}&sp=0'.format(keyword)
                await _search(url)
    ...
    async def create(path: str):
        ...
        async with db_pool.acquire() as db_conn:
            async with db_conn.transaction():
                records = await db_conn.fetch('SELECT keyword from keywords where type=\'{}\''.format(Sogou.type))
                finished_keywords = set([r['keyword'] for r in records])
                keywords.difference_update(finished_keywords)

            async with db_conn.transaction():
                records = await db_conn.fetch('SELECT url from baike_html where type=\'{}\''.format(Sogou.type))
                finished_urls = set([r['url'] for r in records])

        return Sogou(keywords, finished_urls, db_pool)
...
if __name__ == '__main__':
    ...
    crawler: Sogou = loop.run_until_complete(Sogou.create(args.dict))
