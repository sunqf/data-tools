...
class Qihu:
    type = '360_baike'
    # https://baike.so.com/search/?q=%E5%A4%8D%E6%97%A6%E5%A4%A7%E5%AD%A6
    search_address = 'https://baike.so.com'
    item_address = 'https://baike.so.com/doc'
    ...
    async def search_work(self):
        async def _search(url):
            if url not in self.search_urls:
                ...
                if html:
                    html = BeautifulSoup(html, 'html.parser')
                    for tag in html.select('#main > ul > li > h3 > a'):
        ...
        while True:
            ...
            try:
                url = 'https://baike.so.com/search/?q={}'.format(keyword)
                await _search(url)
    ...
    async def create(path: str):
        ...
        async with db_pool.acquire() as db_conn:
            async with db_conn.transaction():
                records = await db_conn.fetch('SELECT keyword from keywords where type=\'{}\''.format(Qihu.type))
                finished_keywords = set([r['keyword'] for r in records])
                keywords.difference_update(finished_keywords)

            async with db_conn.transaction():
                records = await db_conn.fetch('SELECT url from baike_html where type=\'{}\''.format(Qihu.type))
                finished_urls = set([r['url'] for r in records])

        return Qihu(keywords, finished_urls, db_pool)
...
if __name__ == '__main__':
    ...
    crawler: Qihu = loop.run_until_complete(Qihu.create(args.dict))
